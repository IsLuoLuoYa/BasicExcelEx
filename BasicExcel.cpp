...
bool BasicExcel::Load(const char* filename, bool IsOnlyRead)
{
	if (file_.IsOpen()) 
		file_.Close();

	bool OpenRet = false;
	if (IsOnlyRead)		OpenRet = file_.Open(filename, ios_base::in);
	else				OpenRet = file_.Open(filename);
...
bool BasicExcel::Load(const wchar_t* filename, bool IsOnlyRead)
{
	if (file_.IsOpen()) 
		file_.Close();
						...
						case BasicExcelCell::INT32:
						{
							...
							if (cl > c+1)
							{
								...
								for (size_t i=0; c<cl; ++c, ++i)
								{
									...
									pCell->mulrk_.XFRK_[i].RKValue_ = GetRKValueFromInteger(cell->GetInt32());
								}
								--c;
							}
							else
							{
								...
								pCell->rk_.value_ = GetRKValueFromInteger(cell->GetInt32());
							}
							break;
						}
...
bool BasicExcelWorksheet::DealCustomData(int DataStartRow, int DataStartCol)
{
	...
	for (size_t TmpCol = 0; TmpCol < maxCols_; ++TmpCol)
	{
		string Tmpdata(Cell(0, TmpCol)->GetString());
		if (!Tmpdata.empty())
			m_Col_End = TmpCol;
		else
			break;
	...
	if (m_Row_End < m_Row_Start)
		return false;
	if (m_Col_End < m_Col_Start)
		return false;
...
BasicExcelRowData BasicExcelWorksheet::GetRowData_Point(int Row)
{
...
bool BasicExcelCell::Get(char* str) const 
{
	if (type_ == STRING)
	{
		if (VarStr_.empty()) *str = '\0';
		else strcpy(str, &*(VarStr_.begin()));
		return true;
	}
	else return false;
}
...
bool BasicExcelCell::Get(wchar_t* str) const 
{
	if (type_ == WSTRING)
	{
		if (ValWStr_.empty()) *str = L'\0';
		else wcscpy(str, &*(ValWStr_.begin()));
		return true;
	}
	else return false;
}
...
{
	if (type_ == STRING) return VarStr_.size() - 1;
...
int64_t BasicExcelCell::GetInt64() const
{
	return ValInt64_;
}
...
// Set content of current Excel cell to an integer.
void BasicExcelCell::SetInt32(int val) 
{
	type_ = INT32; 
	ValInt32_ = val;
	ValInt64_ = val;
...
void BasicExcelCell::SetInt64(int64_t val)
{
	type_ = INT64;
	ValInt32_ = val;
	ValInt64_ = val;
...
void BasicExcelCell::SetString(const char* str) 
{
	...
	if (length > 0)
	{
		...
		type_ = STRING;
		VarStr_.resize(length + 1);
		strcpy(&*(VarStr_.begin()), str);

		ValWStr_ = BssicExcelUti::s2ws(VarStr_);

		if (BssicExcelUti::IsNum(VarStr_))
		{
			ValInt64_ = atoll(VarStr_.c_str());
			ValInt32_ = (int)ValInt64_;
			ValDouble_ = strtod(VarStr_.c_str(), nullptr);
		}
	}
	else EraseContents();
}
...
void BasicExcelCell::SetWString(const wchar_t* str)	
{
	...
	if (length > 0)
	{
		...
		type_ = WSTRING;
		ValWStr_.resize(length + 1);
		wcscpy(&*(ValWStr_.begin()), str);

		VarStr_ = BssicExcelUti::ws2s(ValWStr_);
